...
if(isset($_POST['Submit']))
{
   if($_GET['id'])
   {
		$sql="UPDATE groups_subcategory SET 
				name='".addslashes($_POST["name"])."',
				groupid='".addslashes($_POST["groupid"])."' where id='".trim($_GET['id'])."'";	
		$q=mysql_query($sql);
		
		header("location:manage_groups_sub.php?msgs=3");
		exit;
	}
	else
	{
		$sql="INSERT INTO groups_subcategory SET 
				name='".addslashes($_POST["name"])."',
				groupid='".addslashes($_POST["groupid"])."'";	
		$q=mysql_query($sql);
		$insertedid=mysql_insert_id();
		
		header("location:manage_groups_sub.php?msgs=1");
		exit;
	}		
}
...
if($_GET['id'])
{
	...
	$SEL="SELECT * from groups_subcategory where id='".$_GET['id']."'";
        ...
        <tbody >
          <tr>
            ...
            <td width="80%" valign="top" align="center"><table width="100%"  border=0 cellpadding="2" cellspacing="2">
                <tr>
                  <td height="35" class="form111"><? if($_GET['id']){?>Edit<? } else {?>Add<? } ?> Sub Group</td>
                </tr>
                <tr>
                  <td height="222" class="formbg" valign="top">
                      ...
                      <table cellspacing="2" cellpadding="2" width=98% border="0" class="t-b">
                        ...
                        <? }?>
						<tr>
                          <td width="19%" height="25" align="right" valign="top"><strong><span class="a">*</span> Main Group:&nbsp;</strong></td>
                          <td height="25" colspan="3" valign="top">
						  <select name="groupid" id="groupid" style="width:250px;" class="solidinput">
							  <?=GetDropdown(id,name,"groups",' where 1=1 order by name asc',stripslashes($ROW->groupid));?>
						   </select>
						  </td>
                        </tr>
                        <tr>
                          <td width="19%" height="25" align="right" valign="top"><strong><span class="a">*</span> Sub Group Name:&nbsp;</strong></td>
