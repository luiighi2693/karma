...
if(isset($_POST['Submit']))
{
   if($_GET['id'])
   {
		$sql="UPDATE options_group SET 
				name='".addslashes($_POST["name"])."'  where id='".trim($_GET['id'])."'";	
		$q=mysql_query($sql);
		
		header("location:manage_option_group.php?msgs=3");
		exit;
	}
	else
	{
		$sql="INSERT INTO options_group SET 
				name='".addslashes($_POST["name"])."'";	
		$q=mysql_query($sql);
		$insertedid=mysql_insert_id();
		
		header("location:manage_option_group.php?msgs=1");
		exit;
	}		
}
...
if($_GET['id'])
{
	...
	$SEL="SELECT * from options_group where id='".$_GET['id']."'";
        ...
        <tbody >
          <tr>
            ...
            <td width="80%" valign="top" align="center"><table width="100%"  border=0 cellpadding="2" cellspacing="2">
                <tr>
                  <td height="35" class="form111"><? if($_GET['id']){?>Edit<? } else {?>Add<? } ?> Option Group</td>
                </tr>
                <tr>
